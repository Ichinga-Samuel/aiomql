"""Comprehensive tests for the Positions module.
...
class TestGetPositionsLive:
    ...
    async def init(self, make_buy_sell_orders):
        ...
        cls = type(self)
        cls.positions = await Positions.get_positions()

    async def test_get_positions_returns_tuple(self):
        """Test get_positions returns a tuple."""
        positions = await Positions.get_positions()
        assert isinstance(positions, tuple)

    async def test_get_positions_contains_trade_positions(self):
    ...
    async def test_get_positions_by_symbol(self):
        ...
        if self.positions:
            ...
            positions = await Positions.get_positions(symbol=symbol)
            for position in positions:
                assert position.symbol == symbol

    async def test_get_positions_by_ticket(self):
        """Test get_positions can filter by ticket."""
        if self.positions:
            ticket = self.positions[0].ticket
            positions = await Positions.get_positions(ticket=ticket)
            assert len(positions) <= 1
            if positions:
                assert positions[0].ticket == ticket

    async def test_get_positions_by_group(self):
        """Test get_positions can filter by group."""
        positions = await Positions.get_positions(group="*USD*")
        for position in positions:
            assert "USD" in position.symbol

    async def test_get_positions_symbol_overrides_ticket(self):
        """Test that symbol filter takes precedence over ticket."""
        if self.positions:
            symbol = self.positions[0].symbol
            # Pass both symbol and ticket, symbol should take precedence
            positions = await Positions.get_positions(symbol=symbol, ticket=99999999)
...
class TestGetPositionByTicketLive:
    ...
    async def test_get_position_by_ticket_returns_trade_position(self):
        """Test get_position_by_ticket returns TradePosition."""
        if self.positions:
            ticket = self.positions[0].ticket
            position = await Positions.get_position_by_ticket(ticket=ticket)
            assert isinstance(position, TradePosition)

    async def test_get_position_by_ticket_correct_ticket(self):
    ...
    async def test_get_position_by_ticket_nonexistent_returns_none(self):
        """Test get_position_by_ticket returns None for nonexistent ticket."""
        position = await Positions.get_position_by_ticket(ticket=999999999999)
        assert position is None

    async def test_get_position_by_ticket_has_required_attributes(self):
...
class TestGetPositionsBySymbolLive:
    ...
    async def test_get_positions_by_symbol_returns_tuple(self):
        """Test get_positions_by_symbol returns a tuple."""
        positions = await Positions.get_positions_by_symbol(symbol="BTCUSD")
        assert isinstance(positions, tuple)

    async def test_get_positions_by_symbol_contains_trade_positions(self):
        """Test get_positions_by_symbol contains TradePosition objects."""
        positions = await Positions.get_positions_by_symbol(symbol="BTCUSD")
        for position in positions:
            assert isinstance(position, TradePosition)

    async def test_get_positions_by_symbol_correct_symbol(self):
        """Test all returned positions have the requested symbol."""
        positions = await Positions.get_positions_by_symbol(symbol="BTCUSD")
        for position in positions:
            assert position.symbol == "BTCUSD"

    async def test_get_positions_by_symbol_nonexistent_returns_empty(self):
        """Test get_positions_by_symbol returns empty tuple for nonexistent symbol."""
        positions = await Positions.get_positions_by_symbol(symbol="NONEXISTENT123")
        assert positions == ()
...
class TestGetTotalPositionsLive:
    ...
    async def test_get_total_positions_returns_int(self):
        """Test get_total_positions returns an integer."""
        total = await Positions.get_total_positions()
        assert isinstance(total, int)

    async def test_get_total_positions_non_negative(self):
        """Test get_total_positions returns non-negative value."""
        total = await Positions.get_total_positions()
        assert total >= 0

    async def test_get_total_positions_matches_get_positions(self):
...
class TestClosePositionLive:
    ...
    async def test_close_position_returns_tuple(self):
        """Test close_position returns a tuple of (bool, OrderSendResult)."""
        if self.positions:
            position = self.positions[0]
            result = await Positions.close_position(position=position)
    ...
    async def test_close_position_success(self):
        ...
        if positions:
            ...
            success, result = await Positions.close_position(position=position)
            if success:
                assert isinstance(result, OrderSendResult)
                assert result.retcode == 10009

    async def test_close_position_by_ticket_returns_tuple(self):
        """Test close_position_by_ticket returns a tuple."""
        # Refresh positions
        positions = await Positions.get_positions()
        if positions:
            ticket = positions[0].ticket
            result = await Positions.close_position_by_ticket(ticket=ticket)
            assert isinstance(result, tuple)
            assert len(result) == 2

    async def test_close_position_by_ticket_nonexistent_raises(self):
        """Test close_position_by_ticket raises InvalidRequest for nonexistent ticket."""
        with pytest.raises(InvalidRequest):
            await Positions.close_position_by_ticket(ticket=999999999999)
...
class TestCloseStaticMethodLive:
    ...
    async def test_close_returns_tuple(self):
        """Test close static method returns tuple of (bool, OrderSendResult)."""
        # Refresh positions
        positions = await Positions.get_positions()
        if positions:
            position = positions[0]
            result = await Positions.close(
...
class TestClosePositionsLive:
    ...
    async def test_close_positions_returns_tuple(self):
        """Test close_positions returns a tuple."""
        positions = await Positions.get_positions()
        result = await Positions.close_positions(positions=positions)
        assert isinstance(result, tuple)

    async def test_close_positions_empty_positions(self):
        """Test close_positions with empty positions returns empty tuple."""
        result = await Positions.close_positions(positions=())
        assert result == ()
...
class TestCloseAllPositionsLive:
    ...
    async def test_close_all_positions_returns_tuple(self):
        """Test close_all_positions class method returns a tuple."""
        result = await Positions.close_all_positions()
        assert isinstance(result, tuple)

    async def test_close_all_positions_contains_order_send_results(self):
...
class TestPositionAttributes:
    ...
    async def test_position_has_ticket(self):
    ...
    async def test_position_has_symbol(self):
    ...
    async def test_position_has_volume(self):
    ...
    async def test_position_has_type(self):
        """Test position has type attribute."""
        if self.positions:
            position = self.positions[0]
            assert hasattr(position, 'type')

    async def test_position_has_price_open(self):
    ...
    async def test_position_has_price_current(self):
    ...
    async def test_position_has_profit(self):
    ...
    async def test_position_has_sl_tp(self):
...
class TestPositionsEdgeCases:
    ...
    async def test_get_positions_empty_when_no_positions(self):
        """Test get_positions returns empty tuple when no positions exist."""
        # Close all positions first
        await Positions.close_all_positions()
        result = await Positions.get_positions()
        # Result should be a tuple (possibly empty)
        assert isinstance(result, tuple)

    async def test_get_positions_with_invalid_group(self):
        """Test get_positions with nonexistent group returns empty."""
        result = await Positions.get_positions(group="NONEXISTENT_GROUP_12345")
        assert result == ()
